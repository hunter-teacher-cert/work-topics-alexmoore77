...
#append the newest block to the chain.
    def new_block(self, proof, previous_hash=None):
        block = {
#index of block=length of chain plus one
            'index': len(self.chain) + 1,
            'timestamp': time(),
#pending transactions from old carry over to new
            'transactions': self.pending_transactions,
#proof of work?
            'proof': proof,
#a hash is an altered string.  What is this hash for?  What does the code below do?
          'previous_hash': previous_hash or self.hash(self.chain[-1]) if self.chain else None,
                }
#pending_transactions = []?  Huh?
        self.pending_transactions = []
#adding to a new chain?
        self.chain.append(block)
        return block

#Search the blockchain for the most recent block.

#The @property decorator adds getters and setters
@property
def last_block(self):
#The -1 in Python seems to refer to the previous list element
        return self.chain[-1]
...
new_transaction(myBlockchain.chain, "Alex","Eric","15")
